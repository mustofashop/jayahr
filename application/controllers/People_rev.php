...
class People_rev extends CI_Controller
{
    ...
    public function entry_idp($tahun, $nrp)
    {
        ...
        if ($masuk != TRUE) {
            ...
            $d['content']   = 'member/member/index_idp2';
            $this->load->view('master', $d);
        }
    }

    public function entry_idp2($tahun, $nrp)
    {
        $masuk  = $this->session->userdata('masuk_k');
        if ($masuk != TRUE) {
            redirect(base_url('login'));
	...
	*/
    ...
    public function rekap_idp()
    {
        ...
        if ($masuk != TRUE) {
            ...
            $d['header']    = 'Rekap IDP  | ' . $tahun . ' | ' . $dtl_bagian->nama_bagian;
            $d['content']   = 'member/member/list_rekap_idp';
            $this->load->view('member', $d);
        }
    }

    function download_rekap_idp($tahun, $nrp)
    {
        $masuk  = $this->session->userdata('masuk_k');
        ...
        if ($masuk != TRUE) {
            ...
            //$data   = $this->enterprise_model->download_data_karyawan($id_perusahaan,$lokasian,$jenis);
            $thn = '2023';
            $data   = $this->master_model->list_idp_rekap($tahun, $nrp);

            if ($data->num_rows() > 0) {
                $pdf = new reportProduct();
                $pdf->setKriteria("cetak_laporan");
                $pdf->setNama("CETAK REKAP IDP");
                $pdf->AliasNbPages();
                $pdf->AddPage("L", "A4");
                $A4[0] = 210;
                $A4[1] = 297;
                $Q[0] = 216;
                $Q[1] = 279;
                $pdf->SetTitle('LAPORAN REKAP IDP');
                ...
                $pdf->SetX(6);
                $pdf->Cell(300, 4, 'LAPORAN REKAP IDP', 0, 0, 'C');
                $pdf->Cell(10, 4, '', 0, 1);
                $pdf->Ln(5);
                ...
                $pdf->Cell($w[3], $h, 'Pertanyaan', 1, 0, 'C');
                $pdf->Cell($w[3], $h, 'Isi IDP', 1, 0, 'C');
                $pdf->Cell($w[3], $h, 'Assesor', 1, 0, 'C');
                $pdf->Cell($w[2], $h, 'Tanggal Entry', 1, 0, 'C');
                ...
                foreach ($data->result() as $dt) {
                    ...
                    $pdf->Cell($w[2], $h, $dt->nama_lengkap, 'LR', 0, 'C', $fill);
                    $pdf->Cell($w[3], $h, $dt->nama_value, 'LR', 0, 'C', $fill);
                    $pdf->Cell($w[3], $h, $dt->isi_idp, 'LR', 0, 'C', $fill);
                    $pdf->Cell($w[3], $h, $dt->insertby, 'LR', 0, 'C', $fill);
                    $pdf->Cell($w[2], $h, $dt->insert_date, 'LR', 0, 'C', $fill);
                    $pdf->Ln();
                    $fill = !$fill;
                    $no++;
                }
                ...
                $pdf->Output('Rekap_IDP_' . $this->master_model->tgl_indo(date('Y-m-d')) . '.pdf', 'D');
            } else {
    ...
    function download_rekap_idp_admin($unit)
    {
        $masuk  = $this->session->userdata('masuk_k');
        $this->load->helpers('print_rekap_helper');
        if ($masuk != TRUE) {
            ...
            //$data   = $this->enterprise_model->download_data_karyawan($id_perusahaan,$lokasian,$jenis);
            $thn = '2023';
            $data   = $this->master_model->list_idp_rekap_admin($unit);
    ...
    function download_rekap_idp_old($tahun, $nrp)
    {
        $masuk  = $this->session->userdata('masuk_k');
        if ($masuk != TRUE) {
            redirect(base_url('login'));
    ...
    function download_rekap_idp_admin_excel($unit)
    {
        $masuk  = $this->session->userdata('masuk_k');
        if ($masuk != TRUE) {
            redirect(base_url('login'));
