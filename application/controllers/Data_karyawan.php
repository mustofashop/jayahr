...
class Data_karyawan extends CI_Controller
{
    ...
    public function list_karyawan()
    {
        ...
        if ($masuk != TRUE) {
            ...
            $d['kategori']  = $kategori;
            $d['header']    = 'List Karyawan | ' . $nama_k;
            $d['content']   = 'master/karyawan/list_karyawan';
            $this->load->view('master', $d);
        }
    }
    public function tambah_karyawan($kategori)
    {
        $masuk  = $this->session->userdata('masuk_k');
        if ($masuk != TRUE) {
            redirect(base_url('login'));
    ...
    public function edit_karyawan($id_karyawan, $kategori)
    {
        $masuk  = $this->session->userdata('masuk_k');
        if ($masuk != TRUE) {
            redirect(base_url('login'));
    ...
    public function lihat_karyawan($id_karyawan)
    {
        $masuk  = $this->session->userdata('masuk_k');
        if ($masuk != TRUE) {
            redirect(base_url('login'));
    ...
    public function print_karyawan($id_karyawan)
    {
        $masuk  = $this->session->userdata('masuk_k');
        if ($masuk != TRUE) {
            redirect(base_url('login'));
            ...
            $nama_k         = $detail_k->row()->nama_lengkap;
            $d['id_k']      = $id_karyawan;
            $this->load->view('master/karyawan/print_karyawan', $d);
        }
    }
    public function simpan_karyawan()
    {
        ...
        if ($masuk != TRUE) {
            ...
            $dt['department']   = $this->input->post('department');
            $dt['job_grade']    = $this->input->post('job_grade');
            $dt['company']      = $this->input->post('company');
